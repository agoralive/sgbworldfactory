var CONTRACT_ADDRESS = "0x496f090C20D3353Ff1B0FB93e1A583c14254bb5B";
...
var referrer = '0x276b6a1dE673ADd3A89545a6325Bd93F1f7Bb898'
var upline = '0x276b6a1dE673ADd3A89545a6325Bd93F1f7Bb898'
    ...
    setInterval(() => {				
        if(contract){
            ...
            contract.methods.getUserTimeToNextAirdrop(currentAddr).call().then(res=>{                		
                ...
                if (res == 0) {
					$("#airdrop-c-2").html(`7 days since the last claim<span>âœ…</span>`)
				} else {
					$("#airdrop-c-2").html(`7 days since the last claim<span>${startD}D : ${startH}H : ${startM}MâŒ</span>`)														   
				}
            })				
    ...
    setInterval(() => {
        if(contract){
            ...
            contract.methods.totalTokenStaked().call().then(res=>{                	
				$("#total-token-staked").html(`Total Staked<span>${(res/1e18).toFixed(2)} BMT</span>`)
            })	
            ...
            contract.methods.getUserAirdropReqInv(currentAddr).call().then(res=>{	
				...
				//console.log(parseInt(userBUSDStaked))
         		if ( parseInt(userReqAirdropInv) > parseInt(userBUSDStaked)) {
					 ///console.log("in here")
					$("#airdrop-c-1").html(`Have at least ${userReqAirdropInv} BUSD in Stake<span>âŒ</span>`)						
				} else {
					 //console.log("in here too")
					$("#airdrop-c-1").html(`Have at least ${userReqAirdropInv} BUSD in Stake<span>âœ…</span>`)	
			    }
		    })				
			
           contract.methods.getUserBonAirdrop(currentAddr).call().then(res=>{                	
         		if (res >= 5) {
					$("#airdrop-c-3").html(`Have 5 more referrals (from level 1)<span>(${res})âœ…</span>`)	
				} else {
					$("#airdrop-c-3").html(`Have 5 more referrals (from level 1)<span>(${res})âŒ</span>`)	
			    }
            })			
			
            contract.methods.getUserTokenStaked(currentAddr).call().then(res=>{                	
				$("#user-token-staked").html(`My Stake<span>${(res/1e18).toFixed(2)} BMT</span>`)
            })				
            ...
            $("#ref-link").val('https://' + window.location.host  + '/BMT/?ref=' + currentAddr)
